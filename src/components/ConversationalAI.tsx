...
export default function ConversationalAI({ role, context }: ConversationalAIProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-lg shadow-lg border border-gray-200">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-gray-200 bg-gradient-to-r from-blue-50 to-indigo-50">
        <Sparkles className="w-5 h-5 text-blue-600" />
        <div>
          <h3 className="font-semibold text-gray-900">AI Assistant</h3>
          <p className="text-xs text-gray-600">Powered by multi-agent intelligence</p>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 min-h-[400px] max-h-[600px]">
        {messages.length === 0 && (
          <div className="text-center py-8">
            ...
            <p className="text-gray-500 mb-6">Ask me anything about your operations</p>
            <div className="space-y-2">
              {suggestedPrompts[role]?.map((prompt, idx) => (
                <button
                  key={idx}
                  onClick={() => handlePromptClick(prompt)}
                  className="block w-full text-left px-4 py-2 text-sm bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg transition-colors"
                >
                  {prompt}
                </button>
              ))}
